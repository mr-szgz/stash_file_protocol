...
retrieved: 2026-04-01T21:31:08.262304+00:00
final_url: https://learn.microsoft.com/en-us/windows/apps/desktop/modernize/desktop-to-uwp-extensions
content_type: text/markdown
-->

---
layout: Conceptual
title: Integrate your desktop app with Windows using packaging extensions - Windows apps | Microsoft Learn
canonicalUrl: https://learn.microsoft.com/en-us/windows/apps/desktop/modernize/desktop-to-uwp-extensions
...
locale: en-us
document_id: b6a7e980-cfd8-6d9c-6cc5-0ca6763fd327
document_version_independent_id: 5f59e641-6576-e10e-99c0-794907285580
updated_at: 2026-02-12T21:44:00.0000000Z
original_content_git_url: https://github.com/MicrosoftDocs/windows-dev-docs-pr/blob/live/hub/apps/desktop/modernize/desktop-to-uwp-extensions.md
gitcommit: https://github.com/MicrosoftDocs/windows-dev-docs-pr/blob/da6a096f3eebf8f1dae14bd09877007515e5dccc/hub/apps/desktop/modernize/desktop-to-uwp-extensions.md
git_commit_id: da6a096f3eebf8f1dae14bd09877007515e5dccc
site_name: Docs
depot_name: MSDN.windows-uwp-hub
page_type: conceptual
toc_rel: ../../package-and-deploy/toc.json
pdf_url_template: https://learn.microsoft.com/pdfstore/en-us/MSDN.windows-uwp-hub/{branchName}{pdfName}
feedback_help_link_type: ''
feedback_help_link_url: ''
word_count: 5392
asset_id: apps/desktop/modernize/desktop-to-uwp-extensions
moniker_range_name: 
monikers: []
item_type: Content
source_path: hub/apps/desktop/modernize/desktop-to-uwp-extensions.md
cmProducts:
...
The features described in this topic require that your app is packaged (has package identity at runtime). That includes packaged apps (see [Create a new project for a packaged WinUI desktop app](../../winui/winui3/create-your-first-winui3-app#packaged-create-a-new-project-for-a-packaged-c-or-c-winui-3-desktop-app)) and packaged apps with external location (see [Grant package identity by packaging with external location](grant-identity-to-nonpackaged-apps)). Also see [Features that require package identity](modernize-packaged-apps).
...
1. In the registry, create a subkey with the name of your desktop app executable file under the **HKEY\_LOCAL\_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options** key.
2. Under this subkey, add the following values:
    - **AppExecutionAliasRedirect** (DWORD): If set to 1, the system will check for an [AppExecutionAlias](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap3-appexecutionalias) package extension with the same name as the executable. If the **AppExecutionAlias** extension is enabled, the packaged app will be activated using that value.
    - **AppExecutionAliasRedirectPackages** (REG\_SZ): The system will redirect only to the listed packages. Packages are listed by their package family name, separated by semicolons. If the special value \* is used, the system will redirect to an **AppExecutionAlias** from any package.
...
In your package manifest, add an [AppExecutionAlias](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap3-appexecutionalias) extension that registers the name of your unpackaged desktop app executable. For example:
...
Find the complete schema reference [here](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-rescap3-desktopappmigration).
...
To do that, you'll specify the [programmatic identifier (ProgID)](/en-us/windows/desktop/shell/fa-progids) of each application from which you want to inherit file associations.
...
Find the complete schema reference [here](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap-filetypeassociation).
...
| MigrationProgId | The [programmatic identifier (ProgID)](/en-us/windows/desktop/shell/fa-progids) that describes the application, component, and version of the desktop application from which you want to inherit file associations. |
...
To use the "windows.firewallRules" extension category (see below), your package needs the **Full Trust Permission Level** restricted capability. See [Restricted capability list](/en-us/windows/uwp/packaging/app-capability-declarations#restricted-capability-list).
...
Find the complete schema reference [here](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-desktop2-firewallrules).
...
Use the [uap6:LoaderSearchPathOverride](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap6-loadersearchpathoverride) extension to declare up to five folder paths in the app package, relative to the app package root path, to be used in the loader search path for the app's processes.

The [DLL search order](/en-us/windows/win32/dlls/dynamic-link-library-search-order) for Windows apps includes packages in the package dependency graph if the packages have execution rights. By default, this includes main, optional and framework packages, although this can be overwritten by the [uap6:AllowExecution](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap6-allowexecution) element in the package manifest.

A package that is included in the DLL search order will, by default, include its *effective path*. For more information about effective paths, see the [EffectivePath](/en-us/uwp/api/windows.applicationmodel.package.effectivepath) property (WinRT) and the [PackagePathType](/en-us/windows/win32/api/appmodel/ne-appmodel-packagepathtype) enumeration (Win32).

If a package specifies [uap6:LoaderSearchPathOverride](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap6-loadersearchpathoverride), then this information is used instead of the package's effective path.

Each package can contain only one [uap6:LoaderSearchPathOverride](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap6-loadersearchpathoverride) extension. That means that you can add one of them to your main package, and then add one to each of your [optional packages, and related sets](/en-us/windows/msix/package/optional-packages).
...
- Show file contents in a Preview pane of File Explorer
...
For more information about the **Kind** field and the values that you can use for this field, see [Using Kind Names](/en-us/windows/desktop/properties/building-property-handlers-user-friendly-kind-names).
...
| Name | The name of the file type association. You can use this name to organize and group file types. The name must be all lower case characters with no spaces. |
| FileType | The relevant file extensions. |
| value | A valid [Kind value](/en-us/windows/desktop/properties/building-property-handlers-user-friendly-kind-names) |
...
If your desktop application defines a [context menu handler](/en-us/windows/desktop/shell/context-menu-handlers), use this extension to register the menu handler.
...
Find the complete schema reference here: [com:ComServer](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-com-comserver) and [desktop4:FileExplorerContextMenus](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-desktop4-fileexplorercontextmenus).

#### Instructions

To register your context menu handler, follow these instructions.

1. In your desktop application, implement a [context menu handler](/en-us/windows/desktop/shell/context-menu-handlers) by implementing the [IExplorerCommand](/en-us/windows/desktop/api/shobjidl_core/nn-shobjidl_core-iexplorercommand) or [IExplorerCommandState](/en-us/windows/desktop/api/shobjidl_core/nn-shobjidl_core-iexplorercommandstate) interface. For a sample, see the [ExplorerCommandVerb](https://github.com/microsoft/Windows-classic-samples/tree/master/Samples/Win7Samples/winui/shell/appshellintegration/ExplorerCommandVerb) code sample. Make sure that you define a class GUID for each of your implementation objects. For example, the following code defines a class ID for an implementation of [IExplorerCommand](/en-us/windows/desktop/api/shobjidl_core/nn-shobjidl_core-iexplorercommand).

    ```cpp
    class __declspec(uuid("00001111-aaaa-2222-bbbb-3333cccc4444")) CExplorerCommandVerb;
    ```
2. In your package manifest, specify a [com:ComServer](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-com-comserver) application extension that registers a COM surrogate server with the class ID of your context menu handler implementation.
    ...
    ```
3. In your package manifest, specify a [desktop4:FileExplorerContextMenus](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-desktop4-fileexplorercontextmenus) application extension that registers your context menu handler implementation.
...
Find the complete schema reference [here](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap-protocol).
...
| TaskId | A unique identifier for your task. Using this identifier, your application can call the APIs in the [Windows.ApplicationModel.StartupTask](/en-us/uwp/api/Windows.ApplicationModel.StartupTask) class to programmatically enable or disable a startup task. |
...
| DropTargetHandler | The class ID of the application that implements the [IDropTarget](/en-us/dotnet/api/microsoft.visualstudio.ole.interop.idroptarget) interface. Files from the removable media are passed to the [Drop](/en-us/dotnet/api/microsoft.visualstudio.ole.interop.idroptarget.drop#Microsoft_VisualStudio_OLE_Interop_IDropTarget_Drop_Microsoft_VisualStudio_OLE_Interop_IDataObject_System_UInt32_Microsoft_VisualStudio_OLE_Interop_POINTL_System_UInt32__) method of your [IDropTarget](/en-us/dotnet/api/microsoft.visualstudio.ole.interop.idroptarget) implementation. |
| Parameters | You don't have to implement the [IDropTarget](/en-us/dotnet/api/microsoft.visualstudio.ole.interop.idroptarget) interface for all content events. For any of the content events, you could provide command line parameters instead of implementing the [IDropTarget](/en-us/dotnet/api/microsoft.visualstudio.ole.interop.idroptarget) interface. For those events, AutoPlay will start your application by using those command line parameters. You can parse those parameters in your app's initialization code to determine if it was started by AutoPlay and then provide your custom implementation. |
| DeviceEvent | The name of a device event that causes users to be prompted with your `ActionDisplayName` and `ProviderDisplayName`. A device event is raised when a device is connected to the PC. Device events begin with the string `WPD` and you can find them listed [here](/en-us/windows/uwp/launch-resume/auto-launching-with-autoplay#autoplay-event-reference). |
| HWEventHandler | The Class ID of the application that implements the [IHWEventHandler](/en-us/windows/desktop/api/shobjidl/nn-shobjidl-ihweventhandler) interface. |
| InitCmdLine | The string parameter that you want to pass into the [Initialize](/en-us/windows/desktop/api/shobjidl/nf-shobjidl-ihweventhandler-initialize) method of the [IHWEventHandler](/en-us/windows/desktop/api/shobjidl/nn-shobjidl-ihweventhandler) interface. |
...
If you want that application to restart after the update completes, call the [RegisterApplicationRestart](/en-us/windows/desktop/api/winbase/nf-winbase-registerapplicationrestart) function in every process that you want to restart.

Each active window in your application receives a [WM_QUERYENDSESSION](/en-us/windows/desktop/Shutdown/wm-queryendsession) message. At this point, your application can call the [RegisterApplicationRestart](/en-us/windows/desktop/api/winbase/nf-winbase-registerapplicationrestart) function again to update the command line if necessary.

When each active window in your application receives the [WM_ENDSESSION](/en-us/windows/desktop/Shutdown/wm-endsession) message, your application should save data and shut down.

Note

Your active windows also receive the [WM_CLOSE](/en-us/windows/desktop/winmsg/wm-close) message in case the application doesn't handle the [WM_ENDSESSION](/en-us/windows/desktop/Shutdown/wm-endsession) message.
...
Find the complete schema reference [here](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-desktop2-appprinter).
...
Find the complete schema reference [here](/en-us/uwp/schemas/appxpackage/uapmanifestschema/element-uap4-sharedfonts).
